...
export default function InfluencerMesajlarPage() {
  ...
  async function load(threadId?: string) {
    try {
      ...
      if (!res.ok || !data?.ok) throw new Error(data?.error ?? "Mesajlar alinamadi");
      ...
      setInfo(error instanceof Error ? error.message : "Mesajlar alinamadi");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="mx-auto flex max-w-[980px] flex-col gap-4 md:flex-row md:gap-6">
      <div className="w-full shrink-0 space-y-2 md:w-72">
        <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-500">Marka mesajlari</p>
        <ul className="rounded-2xl border border-gray-200 bg-white shadow-sm">
          {loading ? (
            <li className="px-4 py-4 text-xs text-gray-500">Mesajlar yukleniyor...</li>
          ) : threads.length ? (
            threads.map((t) => (
              <li
                key={t.id}
      ...
      <div className="flex min-h-[360px] flex-1 flex-col rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
        <p className="text-sm text-gray-500">
          {selected ? `${selected.from} ile konusma` : "Konusma secin, marka ile dogrudan mesajlasin."}
        </p>
        <div className="mt-4 flex-1 space-y-2 overflow-y-auto rounded-xl border border-gray-100 bg-gray-50 p-3">
          {loading ? (
            <p className="text-xs text-gray-500">Konusma yukleniyor...</p>
          ) : messages.length ? (
            messages.map((m) => (
              <div
                key={m.id}
                className={`max-w-[85%] rounded-xl px-3 py-2 text-sm ${
                  m.senderRole === "INFLUENCER" ? "ml-auto bg-pink-600 text-white" : "bg-white text-gray-800"
                }`}
              >
        ...
        <div className="mt-4 flex gap-2 border-t border-gray-100 pt-4">
          <input
            type="text"
            placeholder={activeThread ? "Markaya mesaj yazin..." : "Mesajlasmak icin bir konusma secin"}
            value={text}
